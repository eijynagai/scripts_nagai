...
p17 <- DimPlot(S3, reduction='pca', label=TRUE, pt.size=0.4, label.size=6)+ggtitle(label='PCA')
p18 <- DimPlot(S3, reduction='umap', label=TRUE, pt.size=0.4, label.size=6)+ggtitle(label='UMAP')
p19 <- DimPlot(S3, reduction='tsne', label=TRUE, pt.size=0.4, label.size=6)+ggtitle(label='TSNE')
...
if (findmarkers_test == 'wilcox') {
   ...
   top10 <- S3.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
   write.table(top10, file=paste0(dirname, '/top10.txt'), quote=F, col.names=T, sep='\t', row.names=F, append=F)
   p13 <- DoHeatmap(S3, features = top10$gene) + NoLegend()
   p13

   top20 <- S3.markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_logFC)
   write.table(top20, file=paste0(dirname, '/top20.txt'), quote=F, col.names=T, sep='\t', row.names=F, append=F)
   p14 <- DoHeatmap(S3, features = top20$gene) + NoLegend()
   p14

   } else if (findmarkers_test == 'roc') {

   top10 <- S3.markers %>% group_by(cluster) %>% top_n(n = 10, wt = power)
   write.table(top10, file=paste0(dirname, '/top10.txt'), quote=F, col.names=T, sep='\t', row.names=F, append=F)
   p13 <- DoHeatmap(S3, features = top10$gene) + NoLegend()
   p13

   ### top 20 markers
   top20 <- S3.markers %>% group_by(cluster) %>% top_n(n = 20, wt = power)
   top20
   write.table(top20, file=paste0(dirname, '/top20.txt'), quote=F, col.names=T, sep='\t', row.names=F, append=F)
   p14 <- DoHeatmap(S3, features = top20$gene) + NoLegend()
   p14
