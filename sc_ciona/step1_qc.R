...
if (sctransf){
    ...
    S2 <- SCTransform(S2, vars.to.regress = "Percent_MT", verbose = FALSE)
...
#PCx <- length(attributes(S2[['pca']])$stdev) #automatically
#print(paste0('Number of PCs used:',PCx))


# Determine the dimentionality of the dataset (cannot do on SCTransform)
if (!(sctransf)){
    summary(attributes(S2[['pca']]))
    S2 <- JackStraw(S2, dims=PCx, num.replicate=100, verbose=FALSE)
    S2 <- ScoreJackStraw(S2, dims=1:PCx)
    print(JackStrawPlot(S2, dims=1:PCx, reduction='pca'))
    print(ElbowPlot(S2, ndims=nPCS, reduction='pca') + geom_vline(xintercept=PCx, colour='red'))
}
...
S2 <- RunUMAP(S2, dims=1:PCx, verbose=FALSE)
S2 <- RunTSNE(S2, dims=1:PCx)
...
if(FALSE){
    ...
    for (i in 1:11){
        ...
        # log10
        #VlnPlot(S2, features = markers[i] , slot = "counts", log = TRUE,  pt.size = 0.1,  combine = FALSE)

        # umap position plot
        FeaturePlot(S2, features = genes_list, reduction = "umap", sort.cell = TRUE, pt.size = 0.2, label.size = 2, combine = FALSE)
    }
}
