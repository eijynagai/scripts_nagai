...
$DR_INPUT_FILE = shift(@ARGV);  #DropEst 
...
$Rdata_Vel = $this_output_dir . "Velocyto.Rdata"; #S4, if $NEW_RUN = 1
$Rdata_Vel_marker = $this_output_dir . "Velocyto_marker.Rdata"; #S4_markeronly, if $NEW_RUN = 1
...
if($New_RUN == 1){
   ...
   S4 <- S3 ## Copy. This is not required indeed.
   S5 <- S3 ## reduced to marker gene only
   remove(S3)
